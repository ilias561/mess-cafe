...
export default function ActionsSection({ actionCards }: ActionsSectionProps) {
  ...
  const scrollByAmount = (direction: 'left' | 'right') => {
    if (!scrollerRef.current) return
    const card = scrollerRef.current.querySelector('[data-action-card]')
            ...
            {actionCards.map((card, index) => (
              ...
              >
                <Link
                  ...
                  data-action-card
                  className="group/card flex h-[420px] flex-col overflow-hidden rounded-[2px] border border-line/50 bg-bone-warm transition-all duration-200 ease-out hover:-translate-y-[3px] hover:shadow-[0_14px_28px_rgba(43,43,40,0.10)]"
                >
        ...
        <div className="flex flex-col gap-5 md:hidden">
          {actionCards.map((card, index) => (
            ...
            >
              <Link
                href={`/actions/${card.slug}`}
                className="block overflow-hidden rounded-[2px] border border-line/50 bg-bone-warm"
              >
